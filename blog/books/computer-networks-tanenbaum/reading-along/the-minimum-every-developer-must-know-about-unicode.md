...
How does an email client know how to decode an incoming email? The email body is proceeded by SMTP plaintext headers, one of which specified the email body's encoding.

> Content-Type: text/plain; charset="UTF-8"

A web browser, however, can't expect an HTML page to have that in its HTTP request headers.
...
An HTML page specifies the encoding early enough in the document so that the browser hasn't yet had to decode any non-ASCII characters and that the overhead of restarting the decoding process is low.
...
Also, a web browser will sometimes try to guess the encoding based on the bytes it sees in the document. An encoding may have a characteristic distribution of commonly used bytes, and if the page content falls within this distribution, the browser can be reasonably sure about its encoding.
...
In other words, Internet Explorer was doing AI well before the AI-powered web browsers of the 2020s. Classification is a machine-learning task. If we were to compare the misclassification rate of Internet Explorer's encoding classifier to that of one we could make today in minutes with off-the-shelf neural networks, how would it fare?
